...
namespace Common.Core
{
    ...
    /// Line 9+: Hourly data records
    ///
    /// Hour alignment (all fields strictly time-aligned):
    /// - EPW file runs from Hour=1 (00:00-01:00) to Hour=24 (23:00-24:00) on Dec 31.
    /// - All fields (point-in-time and radiation/illuminance) use the SAME index mapping:
    ///   GetHour(i) returns rawRecords[i] for ALL fields.
    /// - This ensures temperature, humidity, wind, and radiation are strictly time-aligned
    ///   at the same HOY index (e.g., HOY=12 gives both 12:00-13:00 radiation and the
    ///   corresponding meteorological data from the same EPW record).
    /// - HOY=0 corresponds to Jan 1 Hour 1 (00:00-01:00), HOY=8759 to Dec 31 Hour 24.
    /// </summary>
    public class EPWData
    {
        public string City { get; private set; }
        ...
        /// Parses hourly data from EPW file body lines.
        /// 
        /// All fields (point-in-time and radiation/illuminance) are aligned to the same
        /// hour index. GetHour(i) returns rawRecords[i] for ALL fields, ensuring strict
        /// time alignment between meteorological data and radiation values.
        /// </summary>
        private void ParseHourlyData(string[] dataLines)
        {
            int expectedHours = 8760;
            ...
            // Phase 2: All fields aligned to the same hour index (strict time alignment)
            // HourlyRecords[i] = rawRecords[i] for ALL fields.
            // This ensures temperature, humidity, wind, and radiation are always paired
            // from the same EPW record at the same HOY index.
            ApplyHourReordering(rawRecords);
        }

        /// <summary>
        /// Aligns all EPW fields to the same hour index.
        ///
        /// All fields (point-in-time, radiation, illuminance) use the SAME source index.
        /// This ensures temperature, humidity, wind, and radiation are strictly time-aligned
        /// at the same HOY index.
        ///
        /// Mapping: HourlyRecords[i] = rawRecords[i] for ALL fields.
        ///   - Index 0 = first EPW record (Jan 1 Hour 1, 00:00-01:00)
        ///   - Index i = rawRecords[i]
        ///
        /// Time fields (year, month, day, hour, minute, data source):
        ///   - Kept from the original position to preserve the EPW file's datetime.
        /// </summary>
        private void ApplyHourReordering(List<HourlyRecord> rawRecords)
        {
            int n = rawRecords.Count;
            ...
            for (int i = 0; i < n; i++)
            {
                // All fields use the SAME source index (strict time alignment)
                int srcIdx = i;
                // Time fields: keep from original position
                int timeSrcIdx = i;

                var src = rawRecords[srcIdx];
                var timeSrc = rawRecords[timeSrcIdx];
                ...
                var rec = new HourlyRecord
                {
                    ...
                    // Point-in-time fields: from SAME source index as radiation
                    DryBulbTemperature = src.DryBulbTemperature,
                    DewPointTemperature = src.DewPointTemperature,
                    RelativeHumidity = src.RelativeHumidity,
                    WindDirection = src.WindDirection,
                    WindSpeed = src.WindSpeed,
                    TotalSkyCover = src.TotalSkyCover,
                    OpaqueSkyCover = src.OpaqueSkyCover,
                    Visibility = src.Visibility,
                    CeilingHeight = src.CeilingHeight,
                    PresentWeatherObservation = src.PresentWeatherObservation,
                    PresentWeatherCodes = src.PresentWeatherCodes,
                    PrecipitableWater = src.PrecipitableWater,
                    AerosolOpticalDepth = src.AerosolOpticalDepth,
                    SnowDepth = src.SnowDepth,
                    DaysSinceLastSnowfall = src.DaysSinceLastSnowfall,
                    Albedo = src.Albedo,
                    LiquidPrecipitationDepth = src.LiquidPrecipitationDepth,
                    LiquidPrecipitationQuantity = src.LiquidPrecipitationQuantity,

                    // AtmosphericPressure: same index alignment
                    AtmosphericPressure = src.AtmosphericPressure,

                    // Radiation/illuminance fields: from SAME source index
                    ExtraterrestrialHorizontalRadiation = src.ExtraterrestrialHorizontalRadiation,
                    ExtraterrestrialDirectNormalRadiation = src.ExtraterrestrialDirectNormalRadiation,
                    HorizontalInfraredRadiation = src.HorizontalInfraredRadiation,
                    GlobalHorizontalRadiation = src.GlobalHorizontalRadiation,
                    DirectNormalRadiation = src.DirectNormalRadiation,
                    DiffuseHorizontalRadiation = src.DiffuseHorizontalRadiation,
                    GlobalHorizontalIlluminance = src.GlobalHorizontalIlluminance,
                    DirectNormalIlluminance = src.DirectNormalIlluminance,
                    DiffuseHorizontalIlluminance = src.DiffuseHorizontalIlluminance,
                    ZenithLuminance = src.ZenithLuminance,
                };
        ...
        /// For leap years, indices range 0-8783.
        /// 
        /// All fields are strictly time-aligned:
        /// - Index 0 = Jan 1 Hour 1 (00:00-01:00)
        /// - Temperature, humidity, wind, radiation all come from the same EPW record.
        /// </summary>
        /// <param name="hoy">Hour of year, 0-based (0 = Jan 1, Hour 0-1)</param>
        /// <returns>Hourly weather record</returns>
        public HourlyRecord GetHour(int hoy)
        {
            if (hoy < 0 || hoy >= HourlyRecords.Count)
    ...
    /// Single hour weather record from EPW file.
    /// 
    /// All fields are strictly time-aligned to the same hour index.
    /// GetHour(i) returns all fields from the same raw EPW record.
    /// </summary>
    public class HourlyRecord
    {
        public int LineIndex;
        ...
        public int Month;
        public int Day;
        public int Hour;        // 1-24 in original EPW; preserved from original position
        public int Minute;
        public string DataSource;

        // Point-in-time fields (aligned to same index as radiation)
        public double DryBulbTemperature;       // C
        public double DewPointTemperature;      // C
        public double RelativeHumidity;         // %
        public double AtmosphericPressure;      // Pa
        ...
        // Radiation fields (aligned to same index as point-in-time fields)
        public double ExtraterrestrialHorizontalRadiation;  // Wh/m2
        public double ExtraterrestrialDirectNormalRadiation; // Wh/m2
        public double HorizontalInfraredRadiation; // Wh/m2
        public double GlobalHorizontalRadiation;   // Wh/m2
        ...
        // Illuminance fields (aligned to same index as point-in-time fields)
        public double GlobalHorizontalIlluminance; // lux
        public double DirectNormalIlluminance;     // lux
        public double DiffuseHorizontalIlluminance;// lux
        public double ZenithLuminance;             // Cd/m2
