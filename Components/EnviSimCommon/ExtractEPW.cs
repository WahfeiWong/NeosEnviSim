...
namespace EnviSimCommonComponent
{
    ...
    /// Extract EPW component. Reads an EPW file and outputs station information and hourly meteorological data.
    /// 
    /// EPW hour alignment (all fields strictly time-aligned):
    /// - All meteorological fields (temperature, humidity, wind, pressure, sky cover, etc.)
    ///   and radiation fields (GHI, DNI, DHI, HIR, etc.) use the SAME hour index.
    /// - HOY=i corresponds to the i-th raw EPW record for ALL fields.
    /// - This ensures temperature, humidity, wind, and radiation are always paired
    ///   from the same EPW hour (e.g., HOY=12 gives both 12:00-13:00 radiation and
    ///   the corresponding meteorological data from the same record).
    /// - HOY=0 = Jan 1 Hour 1 (00:00-01:00), HOY=8759 = Dec 31 Hour 24.
    /// </summary>
    public class ExtractEPWComponent : GH_Component
    {
        public ExtractEPWComponent()
          : base("Extract EPW", "ExtEPW",
              ...
              "If no HOY is provided, outputs full year data (8760 hours). " +
              "Also extracts monthly ground temperatures at various depths from the GROUND TEMPERATURES section if present. " +
              "Uses standard EnergyPlus EPW column mapping with strict time alignment for all fields.",
              "Neos", "RadSim")
        {
        }
        ...
        protected override void RegisterOutputParams(GH_OutputParamManager pManager)
        {
            pManager.AddTextParameter("EPW Info", "Info",
                ...
                GH_ParamAccess.item);
            pManager.AddNumberParameter("Dry Bulb Temperature", "Tdb",
                "Dry bulb temperature [C] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Dew Point Temperature", "Tdp",
                "Dew point temperature [C] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Relative Humidity", "RH",
                "Relative humidity [%] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Atmospheric Pressure", "P_atm",
                "Atmospheric station pressure [Pa] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("GHI", "GHI",
                "Global horizontal irradiance [Wh/m2] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("DNI", "DNI",
                "Direct normal irradiance [Wh/m2] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("DHI", "DHI",
                "Diffuse horizontal irradiance [Wh/m2] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Extraterrestrial Horizontal Radiation", "ExtHoriz",
                "Extraterrestrial horizontal radiation [Wh/m2] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Extraterrestrial Direct Normal Radiation", "ExtDNI",
                "Extraterrestrial direct normal radiation [Wh/m2] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Horizontal Infrared Radiation", "HIR",
                "Horizontal infrared radiation intensity [Wh/m2] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Wind Speed", "WS",
                "Wind speed [m/s] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Wind Direction", "WD",
                "Wind direction [degrees] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Total Sky Cover", "TSC",
                "Total sky cover [tenths] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Opaque Sky Cover", "OSC",
                "Opaque sky cover [tenths] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Visibility", "Vis",
                "Visibility [km] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Ceiling Height", "Ceil",
                "Ceiling height [m] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Precipitable Water", "PrecipW",
                "Precipitable water [mm] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Aerosol Optical Depth", "AOD",
                "Aerosol optical depth [thousandths] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Snow Depth", "Snow",
                "Snow depth [cm] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Albedo", "Alb",
                "Albedo [-] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddNumberParameter("Liquid Precipitation Depth", "PrecipD",
                "Liquid precipitation depth [mm] for each HOY (strictly time-aligned)", GH_ParamAccess.list);
            pManager.AddGenericParameter("Ground Temps", "GrndTmp",
                "Monthly ground temperatures at various depths (0.5m\u30012m\u30014m). " +
                "Each branch corresponds to one depth (Path = depth index), containing 12 monthly values [C] (Jan-Dec). " +
                "Extracted from EPW GROUND TEMPERATURES section if present; otherwise the tree will be empty.",
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            try
            {
                ...
                // ===================================================================
                // HOY alignment (all fields strictly time-aligned)
                // ===================================================================
                // EPW file: first line = Jan 1 Hour 1 (00:00-01:00), 
                //           last line = Dec 31 Hour 24 (23:00-24:00)
                // 
                // All fields (temperature, humidity, wind, pressure, sky cover, radiation,
                // illuminance, etc.) use the SAME index: HOY=i -> raw line at index i.
                // This ensures all meteorological data and radiation values are strictly
                // paired from the same EPW hour.
                // ===================================================================
                ...
                foreach (int hoy in hoyList)
                {
                    ...
                    // All fields use the SAME index (strict time alignment)
                    int dataIndex = hoy;

                    string dataLine = dataLines[dataIndex];
                    ...
                    if (parts.Length < 22)
                    {
                        AddRuntimeMessage(GH_RuntimeMessageLevel.Warning,
                            $"EPW data line at HOY {hoy} has insufficient columns. " +
                            $"Expected at least 22, got {parts.Length}.");
                        continue;
                    }

                    double ParseField(int idx)
                    {
                        if (idx < parts.Length && double.TryParse(parts[idx], System.Globalization.NumberStyles.Any,
                            System.Globalization.CultureInfo.InvariantCulture, out double val))
                            return val;
                    ...
                    // Point-in-time fields (strictly time-aligned)
                    tdb.Add(ParseField(6));
                    tdp.Add(ParseField(7));
                    rh.Add(ParseField(8));
                    windDir.Add(ParseField(20));
                    windSpeed.Add(ParseField(21));
                    totalSky.Add(ParseField(22));
                    opaqueSky.Add(ParseField(23));
                    visibility.Add(ParseField(24));
                    ceiling.Add(ParseField(25));
                    precipWater.Add(ParseField(28));
                    aerosol.Add(ParseField(29));
                    snowDepth.Add(ParseField(30));
                    albedo.Add(ParseField(32));
                    precipDepth.Add(ParseField(33));

                    // AtmosphericPressure (strictly time-aligned)
                    patm.Add(ParseField(9));

                    // Radiation/illuminance fields (strictly time-aligned, same index)
                    extHoriz.Add(ParseField(10));
                    extDni.Add(ParseField(11));
                    hir.Add(ParseField(12));
                    ghi.Add(ParseField(13));
                    dni.Add(ParseField(14));
                    dhi.Add(ParseField(15));
                }
            }
            catch (Exception ex)
            {
